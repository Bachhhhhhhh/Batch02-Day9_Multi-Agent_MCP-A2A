...
def check_routing(state: LegalState) -> list[Send]:
    """Routing function: conditional dispatch based on keywords."""
...
async def main():
    ...
    try:
        ...
        out_path = os.path.join(os.path.dirname(__file__), "graph_stage4.png")
        with open(out_path, "wb") as f:
            f.write(image_bytes)
        print(f"\nĐã lưu sơ đồ graph thành công vào file '{out_path}'")
    except Exception as e:
        print("Note: Could not draw graph diagram:", e)
